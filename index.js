...
const USDC = {
  chainId: 11155111,
  // USDC Address that supply from Aave Test Assets Not the chainLink USDC
...
const LINK = {
  chainId: 11155111,
  // LINK Address that supply from Aave Test Assets Not the chainLink LINK
...
async function executeSwap(swapRouter, params, signer) {
  ...
  const receipt = await transactionResponse.wait();
  console.log(
    `Transaction Confirmed: https://sepolia.etherscan.io/tx/${receipt.transactionHash}`
  );
  console.log(`-------------------------------`);
...
async function main(swapAmount) {
  ...
  try {
    // Write Approve Token Function and approve the USDC amount
    await approveToken(USDC.address, TOKEN_IN_ABI, inputAmount, signer);

    // Write Get Pool Info Function and get the pool contract
    ...
    // Write Execute Swap Function and return the swapped LINK amount
    const swappedAmount = await executeSwap(swapRouter, params, signer);

    // Write Approve Lending Pool Function and approve the LINK amount
    await approveLendingPool(LINK.address, swappedAmount, signer);

    // Write Supply to Aave Function and supply the LINK amount
...
// Enter Swap Amount
// Here 1 is the amount of USDC that you want to swap
// 1 USDC = 0.0886495633432105 LINK Swapped from Uniswap
main(1);
